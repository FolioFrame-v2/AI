...
작성합니다. 점수는 자기소개서/포트폴리오 평가 기준에 따라 아래 다섯 가지 항목을 종합적으로
고려해 매깁니다.
- 구체성: 경험이 추상적 표현이 아니라 구체적인 상황, 행동, 결과로 서술되어 있는가
- 정량적 성과: 가능한 경우 수치·성과·지표로 임팩트를 보여주는가
- 논리적 흐름: 상황(Situation)-과제(Task)-행동(Action)-결과(Result) 흐름이 자연스러운가
- 직무 연관성: 서술 내용이 지원 직무/역량과 관련되어 있는가
- 표현력·가독성: 문장이 간결하고 맞춤법/어투가 자연스러운가

각 항목이 고르게 우수할수록 높은 점수를, 추상적이거나 부실할수록 낮은 점수를 부여합니다.
"""
